function y=DubinsLL(R0,x0,y0,phi0,R1,x1,y1,phi1,sign)
%% 逆时针起始圆+逆时针终止圆
% 逆时针起始圆
...
if sign==1
    ...
    DrawCircle(R0,x0+R0*cos(phi0+pi/2),y0+R0*sin(phi0+pi/2),phi0-pi/2,Theta+Alpha-pi/2);
    DrawCircle(R1,x1+R1*cos(phi1+pi/2),y1+R1*sin(phi1+pi/2),Theta+Alpha-pi/2+2*pi,2*pi);
    hold on;
    plot([SF(1) FS(1)],[SF(2) FS(2)],'b-','linewidth',1);
    axis equal;
end
y=sqrt((SF(1)-FS(1))^2+(SF(2)-FS(2))^2)+ArcLen(R0,phi0-pi/2,Theta+Alpha-pi/2)+ArcLen(R1,Theta+Alpha-pi/2+2*pi,2*pi);